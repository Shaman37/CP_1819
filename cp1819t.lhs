...
\textbf{Grupo} nr. & 54
\\\hline
a75569 & João Pedro Lima Cardoso Leal	
\\
a75310 & Renato Jorge Cruzinha da Silva
\\
a74658 & Rui Filipe Silva Vieira	
\end{tabular}
\end{center}
...
\aspas{tipos}, a saber,
\begin{code}
data Tipo = V | Vd | Ve | H | Ht | Hb deriving Eq
\end{code}
...
inExpr = either num bop where num (a) = Num a
                              bop (b,(c,d)) = Bop c b d

outExpr :: Expr -> Either Int (Op,(Expr,Expr))
outExpr (Num a) = i1 (a)
outExpr (Bop b c d) = i2 (c,(b,d))
\end{code}

\textbf{Anamorfismo, Catamorfismo e Hilomorfismo}
...
anaExpr :: (a -> Either Int (Op, (a, a))) -> a -> Expr
...
\textbf{Função Calcula}
\par
Traduzindo a função através de um \emph{catamorfismo} obtemos o seguinte diagrama:
...
\par Com esta representação, facilmente vemos que a nossa função \emph{g} ("gene") terá de ir a cada Expr e manter o seu valor inteiro no caso de ser um \emph{Int} e, no caso de termos um tuplo composto por um \emph{Op} e um outro par de valores inteiros \emph{Int}, temos pois apenas de executar umaa operação sobre esses valores, tendo em conta o operador \emph{Op} presente no tuplo.
Assim sendo, chegamos à seguinte definição da função \emph{calcula}:

\begin{code}
calcula :: Expr -> Int
calcula = cataExpr(either id g) where g (Op "+",(a,b)) = a + b 
                                      g (Op "*",(a,b)) = a * b
                                      g (Op "-",(a,b)) = a - b
\end{code}
...
\textbf{Função Show'}
\par
O problema agora encontra-se em passar uma expressão \emph{Expr} para a respetiva \emph{String} que a representa.
Aplicando um \emph{catamorfismo}, e representando-o através de um diagrama obtemos:
...
\par
Através desta representação, facilmente inferimos o nosso gene \emph{g} que para o caso de ser \emph{Int} o representa como uma \emph{String} (através da função auxiliar \emph{s\_gen1}), e no caso de ser o tuplo composto por um operador \emph{Op} e outro tuplo de duas \emph{String}, temos apenas de representar a expressão consoante o operador \emph{Op} do tuplo (através da função auxiliar \emph{s_gen2}).
\par
Concluindo, obtemos a seguinte função \emph{show'}:

\begin{code}      
show' :: Expr -> String
show' = cataExpr(either s_gen1 s_gen2) 
\end{code}

\par
\emph{Funções Auxiliares:}
\par
\begin{code}
s_gen1 :: Int -> String
s_gen1 i = show i

s_gen2 :: (Op,(String,String)) -> String
s_gen2 (Op "+",(s1,s2)) = "(" ++ s1 ++ " + " ++ s2 ++ ")"
s_gen2 (Op "*",(s1,s2)) = "(" ++ s1 ++ " * " ++ s2 ++ ")"
s_gen2 (Op "-",(s1,s2)) = "(" ++ s1 ++ " - " ++ s2 ++ ")"
\end{code}

\textbf{Função Compile}
\par
A função \emph{compile} transforma uma \emph{String} em \emph{Código} propriamente dito, contudo o problema passa por descobrir como usar as expressões de {\emph{cata},\emph{ana} e \emph{hilo}}-morfismos desenvolvidas acima para uma \emph{Expr} quando o tipo recebido é uma \emph{String}. Assim sendo, recorremos à função auxiliar \emph{ReadExp} que funciona como um parser da \emp{String} recebida, produzindo apartir desta uma lista de tuplos, com hipóteses de \emph{Expr} que sejam capaz de a representar, na qual a melhor hipótese é sempre a cabeça da lista devolvida, assim como a primeira entrada desse tuplo.
\par 
Assim sendo, procedamos a uma representação da função \emph{compile}, como um catamorfismo, através de um diagrama:
\begin{eqnarray*}
\xymatrix@@C=3cm{
    |String|
    \ar[r]_-{|p1 . head . readExp|}
&
    |Expr| 
    \ar[d]_-{|compile|}
    \ar@@/^1pc/[r]^{|outExpr|}
...
\\ 
   &  |Int| 
   & |Int + (Op >< (Código >< Código))| 
      \ar[l]^-{g}
}
\end{eqnarray*}

\par
Podemos assim, através deste diagrama facilmente definir o "gene" \emph{g} que para valores inteiros \emph{Int} os representa como \emph{Código} e no caso de ser um tuplo composto por um operador \emph{Op} e um outro tuplo de dois \emph{Código}, temos então simplesmente de os concatenar juntamente com a operação correta que depende, novamente, do operador \emph{Op}.

\begin{code}
compile :: String -> Codigo
compile = cataExpr(either f g) . p1 . head . readExp where f a = ["PUSH " ++ show a]
                                                           g (Op "+",(a,b)) = a ++ b ++ ["ADD"]
                                                           g (Op "-",(a,b)) = a ++ b ++ ["SUB"]
                                                           g (Op "*",(a,b)) = a ++ b ++ ["MUL"]
\end{code}
...
inL2D = either unid comp where unid (a) = Unid a
                               comp (a,(b,c)) = Comp a b c

outL2D :: X a b -> Either a (b, (X a b,X a b))
outL2D (Unid a) = i1 (a)
outL2D (Comp b c d) = i2 (b,(c,d))
\end{code}
...
cataL2D :: (Either b1 (b2, (d, d)) -> d) -> X b1 b2 -> d
cataL2D g = g . (recL2D (cataL2D g)) . outL2D

anaL2D :: (a1 -> Either a2 (b, (a1, a1))) -> a1 -> X a2 b
...
Para calcular as coordenadas iniciais das caixas no plano apenas temos de adicionar o resultado do catamorfismo \textit{dimen} com a origem segundo o seu \textit{Tipo}. Para tal utilizámos um anamorfismo que ao fazer \textit{  distl . (outL2D X id)) } e aplicar o "gene" obtemos uma L2D com as caixas e respetivas origens . 


\begin{code}
calcOrigins :: (L2D,Origem) -> X (Caixa,Origem) ()
calcOrigins = anaL2D ((id -|- (split (!) calcOrigins_abs)) . distl . (outL2D >< id))
\end{code}
...
\begin{code}
dimen1 :: Caixa -> (Float,Float)
dimen1 ((x,y),(_,_)) = (fromIntegral x, fromIntegral y)

dimen2 :: (Tipo,((Float,Float),(Float,Float))) -> (Float,Float)
dimen2 (t,((e1,e2),(d1,d2))) | elem t [H,Hb,Ht] = (e1+d1,max e2 d2)
                             | elem t [V,Ve,Vd] = (max e1 d1,e2+d2)
                             
dimen :: L2D -> (Float,Float)
dimen = cataL2D( either dimen1 dimen2)

calcOrigins_abs :: ((Tipo,(L2D,L2D)),Origem) -> ((L2D,Origem),(L2D,Origem))
calcOrigins_abs ((H,(e,d)),(ox,oy)) = ((e,(ox,oy)),(d,(ox+difx,oy-dify)))
  where difx = (fst $ dimen e)
        dify = (snd $ dimen d)/2 - (snd $ dimen e)/2
calcOrigins_abs ((Hb,(e,d)),(ox,oy)) = ((e,(ox,oy)),(d,(ox+difx,oy)))
  where difx = (fst $ dimen e)
calcOrigins_abs ((Ht,(e,d)),(ox,oy)) = ((e,(ox,oy)),(d,(ox+difx,oy-dify)))
  where difx = (fst $ dimen e)
        dify = (snd $ dimen d) - (snd $ dimen e)
calcOrigins_abs ((V,(e,d)),(ox,oy)) = ((e,(ox,oy)),(d,(ox-difx,oy+dify)))
  where difx = (fst $ dimen d)/2 - (fst $ dimen e)/2
        dify = (snd $ dimen e)
calcOrigins_abs ((Ve,(e,d)),(ox,oy)) = ((e,(ox,oy)),(d,(ox,oy+dify)))
  where dify = (snd $ dimen e)
calcOrigins_abs ((Vd,(e,d)),(ox,oy)) = ((e,(ox,oy)),(d,(ox-difx,oy+dify)))
  where difx = (fst $ dimen d) - (fst $ dimen e)
        dify = (snd $ dimen e)
\end{code}
...
De forma agrupar as caixas utilizamos um catamorfismo que no caso de \textit{Unid a} aplica \textit{singl . swap a} e em caso de \textit{(Comp t,(b,c))} aplica \textit{conc . \pi} .
...
Chegando ao seguinte diagrama:

\begin{eqnarray*}
\xymatrix@@C=2cm{
    |{CAIXA+(TIPO^{(X CAIXA TIPO)}}|
           \ar[d]_-{cataL2D (g)}
&
...
\\
     CAIXA + TIPO ^{(X (CAIXA >< TIPO)}
&
     |CAIXA + TIPO ^{(X (CAIXA X TIPO) X (X (CAIXA X TIPO)}|
           \ar[l]^-{|g|}
}
\end{eqnarray*}
...
desenha_caixa :: (Origem,Caixa) -> G.Picture
desenha_caixa (o,((l,a),(t,c))) = crCaixa o (fromIntegral l) (fromIntegral a) t c

mostra_caixas :: (L2D,Origem) -> IO()
mostra_caixas = display . G.pictures . (map desenha_caixa) . agrup_caixas . calcOrigins

caixasAndOrigin2Pict = G.pictures . (map desenha_caixa) . agrup_caixas . calcOrigins
...
\par Para resolver este problema, foi necessário desenvolver o somatório de \emph{cos x} em prol de encontrar as expressões de recursividade mútua que o compõem, e seguidamente usar a \emph{Regra da Algibeira} mencionada no enunciado para completar o exercício. Para tal, usufruímos do Anexo \textbf{B} que nos mostra um problema semelhante.


\par \par Tendo isso em conta, começamos por expressar o somatório e os seus valores, no caso de \emph{i = 0} e \emph{i = (n+1)}, de onde facilmente retiramos a nossa expressão \emph{h x n}.

\begin{figure}[H]
    \includegraphics[width=8cm]{cos_linha_1}
\end{figure}


\par Assim sendo, podemos escrever a expressão acima da seguinte forma:


\begin{figure}[H]
    \includegraphics[width=8cm]{cos_linha_2}
\end{figure}


\par Olhemos agora para a expressão h, de onde facilmente retiramos os seus valores para \emph{i = 0} e \emph{i = (n+1)}, e onde podemos também desenvolver a expressão \emph{h}, de forma a achar a expressão de recursividade mútua \emph{s}.


\begin{figure}[H]
    \includegraphics[width=8cm]{h_l}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=8cm]{h_2}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=8cm]{h_3}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=8cm]{h_4}
\end{figure}


\par Por fim, resta-nos descobrir uma expressão \emph{s} de forma a chegar À solução pretendida. Assim, e olhando para o termo \emph{((2n + 4)(2n + 3))}, podemos inferir que \emph{s = 2n + 3} de onde retiramos que \emph{((2n + 4)(2n + 3)) = (s + 1)(s)}. Procedamos então a desenvolver a mesma:


\begin{figure}[H]
    \includegraphics[width=8cm]{s_1}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=8cm]{s_2}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=8cm]{s_3}
\end{figure}


\par Chegando à conclusão que:


\begin{figure}[H]
    \includegraphics[width=8cm]{p3_final}
\end{figure}


\par Onde, seguindo a regra da algibeira podemos então chegar à solução.
...
cos' x = prj . for loop init where
   init = (1,(-x^2)/2,3)
   loop (c,h,s) = (c + h,(-x^2)/(s * (s + 1)) * h, s + 2)
   prj (c,h,s) = c 
\end{code}
...
check = cataFS((uncurry (&&)) . split (nr . map p1) (and . rights . (fmap p2)))
